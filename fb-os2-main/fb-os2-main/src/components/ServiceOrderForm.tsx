...
export function ServiceOrderForm() {
  ...
  const getServicePrice = (equipmentType: string) => {
    if (!equipmentType) return '';
    return servicePrices.cleaning_prices[equipmentType] || '';
  };
          ...
          <div key={index} className="space-y-6 border-l-4 border-blue-200 pl-4 mt-6">
            ...
            {(service.service_type === 'installation' || service.service_type === 'cleaning') && (
              ...
              <select
                ...
                register(`services.${index}.equipment_type`).onChange(e);
                if (service.service_type === 'cleaning') {
                  const price = getServicePrice(value);
                  setValue(`services.${index}.custom_service_value`, price);
                }
                }}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            ...
            {service.service_type === 'cleaning' && (
              ...
              <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                (Valor pode ser ajustado conforme necessário)
                </span>
              </label>
